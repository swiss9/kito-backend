...
function deduplicateAndRankTorrents(torrents) {
  const seen = new Map();
  torrents.forEach(t => {
    const cleanKey = t.name
      .replace(/\[.*?\]|\(.*?\)/g, '')
      .replace(/\b(1080p|720p|2160p|HEVC|x264|x265|BluRay|WEBRip)\b/gi, '')
      .replace(/\s+/g, ' ')
      .trim()
      .toLowerCase();
    const existing = seen.get(cleanKey);
    if (!existing || t.seeders > existing.seeders || (t.seeders === existing.seeders && parseSize(t.size) > parseSize(existing.size))) {
      seen.set(cleanKey, t);
    }
  });
  return Array.from(seen.values()).sort((a, b) => b.seeders - a.seeders);
}
...
function filterBestTorrents(torrents) {
  ...
  function extractEpisodeInfo(name) {
    ...
    if (/batch|complete|全集|season pack/i.test(name)) {
      isBatch = true;
    }
...
app.get('/api/torrents', async (req, res) => {
  try {
    ...
    const ranked = deduplicateAndRankTorrents(filtered);
