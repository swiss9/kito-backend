...
async function scrape1337x(title) {
  const domains = ['1337x.to', '1337x.st', 'x1337x.se'];
  for (let domain of domains) {
    try {
      const searchUrl = `https://${domain}/search/${encodeURIComponent(title)}/1/`;
      ...
      console.warn(`1337x (${domain}) failed:`, err.message);
    }
  }
  return [];
}
...
function filterBestTorrents(torrents) {
  ...
  // Strip resolution and quality tags before extracting episode numbers
  ...
  function extractEpisodeInfo(name) {
    ...
    // 1. Check for explicit batch keywords
    if (/batch|complete|全集|season pack|all episodes/i.test(name)) {
      isBatch = true;
    }

    // 2. Try to extract range patterns
    const rangePatterns = [
      /(\d+)\s*[-–]\s*(\d+)/,
      /[Ss](\d+)[Ee](\d+)\s*[-–]\s*[Ee]?(\d+)/,
    ...
    // 3. Try single episode if no range
    ...
    // If we have a range but start > end, fallback to single episode
  ...
  for (let [key, group] of epGroups) {
    // Keep the entire group sorted by seeders, but we'll keep the top entry for that episode
    // Actually we want to keep all unique resolutions? The user wants to keep multiple resolutions per episode.
    // We need to keep one entry per resolution per episode.
    // However the request says "group releases into an Episode Map that holds multiple resolutions per episode sorted by seeders".
    // So we should keep all entries in the group, not just the top one.
    // But we also want to avoid duplicate episode numbers with different quality.
    // So we group by episode number and within that by resolution/quality tags.
    // Simpler: just keep the sorted list per episode, but since we need to return a flat list, we can keep the top one per resolution.
    // But the request says "holds multiple resolutions per episode sorted by seeders" – so we need to return multiple entries per episode?
    // Actually the user says "Stop discarding valid torrents with sorted[0]. Instead, group releases into an Episode Map that holds multiple resolutions per episode sorted by seeders."
    // That means we shouldn't just take the first one; we should keep all that are distinct in resolution/quality.
    // I'll implement: for each episode, group by a "quality key" (like resolution and source) and pick the best for each quality.
    // For simplicity, we'll just keep all entries and deduplicate by name, but that may still produce duplicates.
    // Since the user wants to see multiple resolutions, we should keep distinct ones.
    // Let's de-duplicate by name after sorting.
    const seenNames = new Set();
    const sortedGroup = group.sort((a, b) => {
      if (a.seeders !== b.seeders) return b.seeders - a.seeders;
      return parseSize(b.size) - parseSize(a.size);
    });
    // Keep only unique names (or unique normalized names)
    const unique = [];
  ...
  // Group batches by normalized title (without episode info)
  ...
  for (let [key, group] of batchGroups) {
    ...
    // Keep only unique batches (by normalized name)
    const seen = new Set();
    const unique = [];
    for (let t of sorted) {
      const key = t.normalized;
  ...
  // Combine: batches first, then singles
  const combined = [...bestBatches, ...bestSingles];

  // Sort: batches first, then by episode number (if available)
  combined.sort((a, b) => {
    ...
    for (let cat of targets) {
      ...
      if (config.source === 'anilist') {
        ...
        if (quality !== 'Any' || source !== 'Any' || language !== 'Any') {
          ...
          if (quality && quality !== 'Any') {
            // Map quality to certification? Not implemented.
          }
          if (source && source !== 'Any') {
            // Map source to watch providers? Not implemented.
          }
...
app.get('/api/torrents', async (req, res) => {
  try {
    ...
    // If episode is provided, append it to query
    let searchTitle = title;
    if (episode) {
      searchTitle = `${title} ${episode}`;
    } else {
      // Default to batch search if no episode specified
      searchTitle = `${title} batch`;
    }

    const result = await getTorrents(searchTitle, category, source);
