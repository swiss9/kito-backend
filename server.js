...
function getRecommendations(bookmarks) {
  ...
  bookmarks.forEach(b => {
    const found = b.mediaId ? null : null;
    if (b.category) {
  ...
  const topGenres = sortedGenres.slice(0, 3).map(g => g[0]);

  const topCategories = Object.entries(mediaTypes).sort((a, b) => b[1] - a[1]).slice(0, 2).map(c => c[0]);

  return { genres: topGenres, categories: topCategories };
}
...
async function getForYouRecommendations(bookmarks) {
  ...
  for (let catId of targetCategories) {
    ...
    if (config.metadataProvider === 'anilist') {
      const genreFilter = profile.genres.length > 0 ? profile.genres[0] : null;
      const query = `
      ...
      const variables = { type, genre: genreFilter, perPage: 4 };
      try {
