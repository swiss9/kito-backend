...
async function fetchTorrentAPI(title) {
  const url = `https://torrentapi.org/pubapi_v2.php?app_id=kito&mode=search&search_string=${encodeURIComponent(title)}&format=json&limit=50`;
  const res = await fetch(url, {
...
async function getTorrents(title, category, source = 'auto', episode = null) {
  ...
  const fetchFn = async () => {
    ...
    // Determine category type for fallback
    const animeLike = ['Anime', 'Manga', 'Tokusatsu'];
    const movieLike = ['Hollywood', 'Bollywood', 'Animation', 'Asian'];

    if (animeLike.includes(category)) {
      fallbackChain = ['torrentclaw', 'nyaa', 'torrentapi'];
    } else if (movieLike.includes(category)) {
      // For movies, we don't want "batch" appended (handled in caller)
      fallbackChain = ['torrentclaw', 'torrentapi'];
      // If it's a TV show, we could add eztv but we don't know – we'll add it as a last resort
      fallbackChain.push('eztv');
    } else {
      fallbackChain = ['torrentclaw', 'torrentapi'];
    }
    ...
    for (let src of fallbackChain) {
      try {
        if (src === 'torrentclaw') {
          ...
          usedSource = 'nyaa';
        } else if (src === 'torrentapi') {
          torrents = await fetchTorrentAPI(title);
          usedSource = 'torrentapi';
        } else if (src === 'eztv') {
...
app.get('/api/torrents', async (req, res) => {
  try {
    ...
    // Determine search query: for anime-like categories, default to "batch" if no episode specified.
    // For movies, search the title as is.
    ...
    // else: searchTitle remains the raw title (for movies)
