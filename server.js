...
// Sequel / spin-off keywords (used as fallback when relations are missing)
...
async function fetchAniList(query, variables) {
  ...
  if (data.errors) {
    console.error('AniList errors:', JSON.stringify(data.errors));
...
function normalizeJikanMedia(item, categoryId) {
  ...
  return {
    ...
    category: categoryId,
    relations: [] // Jikan doesn't provide relations via search
  };
}
...
function getForbiddenRelationTitles(media) {
  ...
  // Add common sequel keywords as fallback, but only if media title doesn't already contain them
...
function validateAndScoreRelease(release, media) {
  ...
  // Title match using word boundaries (any media title must appear as whole word in release title)
  ...
  if (releaseSeason !== mediaSeason) {
    ...
    if (mediaSeason === 1) {
      // For season 1, reject if release title clearly indicates a later season
      const seasonIndicators = [`s0?2`, `season 2`, `2nd season`];
      const hasLaterSeason = seasonIndicators.some(ind => new RegExp(`\\b${ind}\\b`, 'i').test(relTitleNorm));
      if (hasLaterSeason) return null;
    } else {
  ...
  // ---- Relation/forbidden exclusion with word boundaries ----
  ...
  for (const forb of forbiddenTitles) {
    ...
    // If the release contains a forbidden term, we only reject if none of the media titles contain that term as a whole word.
  ...
  const exactTitleMatch = mediaTitles.some(mt => wordBoundaryMatch(releaseTitle, mt) && releaseTitle === mt);
    ...
    for (const catId of categories) {
      ...
      if (config.metadataProvider === 'anilist') {
        ...
        try {
          ...
          anilistItems = (data.Page.media || []).map(item => {
            const media = normalizeAniListMedia(item, catId, []);
            return mediaToCard(media);
          });
        } catch (err) {
          console.warn(`AniList failed for "${q}":`, err.message);
        }

        if (anilistItems.length === 0) {
          try {
            const jikanData = await searchJikan(q);
            jikanItems = jikanData.map(item => {
              const media = normalizeJikanMedia(item, catId);
              return mediaToCard(media);
            });
          } catch (err) {
            console.warn(`Jikan failed for "${q}":`, err.message);
          }
        }

        const animeItems = anilistItems.length > 0 ? anilistItems : jikanItems;

        if (animeItems.length === 0 && TMDB_API_KEY) {
          try {
            ...
            const tmdbItems = tmdbResults.map(item => {
              const media = normalizeTmdbMedia(item, catId);
              return mediaToCard(media);
            });
            allResults.push(...tmdbItems);
          } catch (err) {
            console.warn(`TMDB fallback failed for "${q}":`, err.message);
          }
        } else {
          allResults.push(...animeItems);
        }
      }

      if (config.metadataProvider === 'tmdb' && TMDB_API_KEY) {
        try {
          ...
          console.warn(`TMDB error for ${catId}:`, err.message);
        }
      }
    }
    ...
    if (cached) {
      ...
      if (provider === 'anilist') {
        ...
        if (apiRes.ok) {
          ...
          rawMedia = data.data;
          if (rawMedia) {
            mediaObject = normalizeJikanMedia(rawMedia, categoryId);
          }
        }
