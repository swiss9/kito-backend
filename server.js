...
function parseReleaseName(name) {
  try {
    const parsed = anitomy.parseSync(name);
    let ep = null;
    if (parsed.episode_number) {
      ep = Array.isArray(parsed.episode_number)
        ? parseInt(parsed.episode_number[0])
        : parseInt(parsed.episode_number);
    }
    let season = null;
    if (parsed.anime_season) {
      season = Array.isArray(parsed.anime_season)
        ? parseInt(parsed.anime_season[0])
        : parseInt(parsed.anime_season);
    }
    return {
      episodeNumber: isNaN(ep) ? null : ep,
      season: isNaN(season) ? null : season,
      releaseGroup: parsed.release_group || null,
      resolution: parsed.video_resolution || null,
      source: parsed.source || null,
      title: parsed.anime_title || null
    };
  } catch (e) {
    return { episodeNumber: null, season: null, releaseGroup: null, resolution: null, source: null, title: null };
  }
}
...
function getReleaseGroup(name) {
  try {
    const parsed = anitomy.parseSync(name);
    return parsed.release_group || null;
  } catch (e) {
...
function extractEpisodeNumberFromParsed(parsed, name, mediaTitles = []) {
  if (parsed && parsed.episodeNumber !== null) return parsed.episodeNumber;
  if (!name) return null;
  let clean = name
    .replace(/\[.*?\]|\(.*?\)/g, ' ')
...
function parseSourceWithParsed(parsed, name) {
  if (parsed && parsed.source) return parsed.source.toLowerCase();
...
function validateAndScoreRelease(release, media) {
  ...
  if (range) {
    ...
    const ep = extractEpisodeNumberFromParsed(parsed, name, mediaTitles);
    episodeStart = ep;
    episodeEnd = ep;
  }
  ...
  const sourceType = parseSourceWithParsed(parsed, name);
...
function filterReleasesByQuality(releases, quality) {
  ...
  if (quality === 'Best') {
    const best = releases.reduce((a, b) => a.quality > b.quality ? a : b, null);
    return best ? [best] : releases;
  }
  let qNum;
  if (quality.toLowerCase() === '4k' || quality.toLowerCase() === '2160p') {
    qNum = 2160;
  } else {
    qNum = parseInt(quality);
  }
  if (isNaN(qNum)) return releases;
  return releases.filter(r => r.quality === qNum);
}
...
app.get('/api/releases', async (req, res) => {
  try {
    ...
    const highConfidence = filtered.filter(r => r.confidence === 'high');
    const medConfidence = filtered.filter(r => r.confidence === 'medium');
    const lowConfidence = filtered.filter(r => r.confidence === 'low');
