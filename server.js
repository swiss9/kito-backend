...
function deduplicateAndRank(torrents, media) {
  ...
  const filtered = enriched.filter(t => {
    ...
    let threshold = media.category === 'anime' ? 0.08 : 0.15;
    if (t.isBatch) {
      threshold = 0.04;
      const hasExactTitle = titleVariants.some(v => name.includes(v) && name.split(/\s+/).some(w => w === v));
      if (hasExactTitle) return true; // skip similarity if exact title word appears
    }
