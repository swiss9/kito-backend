...
function filterBestTorrents(torrents) {
  ...
  const enriched = torrents.map(t => {
    ...
    const batchRegex = /batch|complete|全集|season pack|1-|\d+\s*-\s*\d+/i;
    if (batchRegex.test(t.name)) {
      const rangeMatch = t.name.match(/(\d+)\s*[-–]\s*(\d+)/) || t.name.match(/[Ss](\d+)[Ee](\d+)\s*[-–]\s*[Ee]?(\d+)/);
      if (rangeMatch) {
        if (rangeMatch.length === 3) {
          episodeRange = { start: parseInt(rangeMatch[1]), end: parseInt(rangeMatch[2]) };
        } else if (rangeMatch.length === 4) {
          episodeRange = { start: parseInt(rangeMatch[2]), end: parseInt(rangeMatch[3]) };
        }
        isBatch = true;
      } else {
        isBatch = true;
      ...
      for (let pattern of epPatterns) {
        ...
        if (match) {
          if (match.length === 3) {
            ...
            if (!isNaN(num) && num > 0 && num < 1000) episode = num;
          }
          break;
        }
      }
  ...
  singles.forEach(t => {
    const key = t.episode !== null ? `${t.normalized}_ep${t.episode}` : `noep_${t.normalized}`;
  ...
  // Return all deduplicated results
