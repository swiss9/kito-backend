...
async function searchJikan(query) {
  // Normalize query: remove diacritics, spaces
...
function validateAndScoreRelease(release, media) {
  ...
  // ---- Hardcoded exclusions for specific series ----
  // Naruto (2002) – reject sequels/spin-offs
  if (media.title.toLowerCase().includes('naruto') && media.year === 2002) {
    const lowerRel = releaseTitle.toLowerCase();
    if (lowerRel.includes('shippuuden') || lowerRel.includes('shippuden') || 
        lowerRel.includes('boruto') || lowerRel.includes('next generations') || 
        lowerRel.includes('rock lee')) {
      return null;
    }
  }
  ...
  // Relation exclusion (token-level)
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    for (const catId of categories) {
      ...
      // ----- ANIME category (use AniList, fallback to Jikan, then TMDB) -----
      if (config.metadataProvider === 'anilist') {
        ...
        // 2. If AniList returned nothing, try Jikan
        ...
        // Use AniList results if available, else Jikan
        const animeItems = anilistItems.length > 0 ? anilistItems : jikanItems;

        // 3. If still empty, fallback to TMDB (last resort)
