...
function filterBestTorrents(torrents) {
  ...
  // Normalize and extract episode info
  const enriched = torrents.map(t => {
    let normalized = t.name
      .replace(/\[.*?\]/g, '')
      .replace(/\(.*?\)/g, '')
      .replace(/\b(1080p|720p|2160p|WEB-DL|WEBRip|BluRay|x264|x265|HEVC|HDR|10bit|Dual Audio|Multi Sub)\b/gi, '')
      .replace(/\s+/g, ' ')
      .trim();
    ...
    // Try to extract episode number or range
    // Patterns: S01E01, Ep 1, Episode 1, E01, - 1 -, (1), etc.
    const epPatterns = [
      /[Ss](\d+)[Ee](\d+)/,                     // S01E01
      /[Ee]p(?:isode)?\s*(\d+)/i,                // Ep 1, Episode 1
      /[Ee](\d+)/,                               // E01
      /(?:^|\s)-?\s*(\d+)\s*-\s*(\d+)\s*(?:$|\s)/, // 1-90
      /(?:^|\s)(\d+)\s*-\s*(\d+)\s*(?:$|\s)/,     // 1 - 90
      /[\(\[](\d+)-(\d+)[\)\]]/,                 // (1-90)
    ];

    for (let pattern of epPatterns) {
      const match = t.name.match(pattern);
      if (match) {
        if (match.length === 3) {
          // season+ep or range
          if (!isNaN(match[1]) && !isNaN(match[2])) {
            episodeRange = { start: parseInt(match[1]), end: parseInt(match[2]) };
          }
        } else if (match.length === 2) {
          const num = parseInt(match[1]);
          if (!isNaN(num)) {
            if (episodeRange && episodeRange.start === num) {
              // already set
            } else {
              episode = num;
            }
    ...
    // If episodeRange found, use it
    if (episodeRange) {
      // We'll treat this as a batch
      return { ...t, normalized, episode: null, episodeRange, isBatch: true };
    }

    // Check if it's a batch (without range)
    const isBatch = /(batch|complete|全集|season pack)/i.test(t.name);

    return { ...t, normalized, episode, episodeRange, isBatch };
  });

  // Separate batch releases and single episodes
  const batches = enriched.filter(t => t.isBatch || t.episodeRange);
  const singles = enriched.filter(t => !t.isBatch && !t.episodeRange);

  // For singles, group by normalized title and episode number (if available)
  const epGroups = new Map();
  singles.forEach(t => {
    const key = t.normalized + (t.episode !== null ? `_ep${t.episode}` : '');
    if (!epGroups.has(key)) epGroups.set(key, []);
    epGroups.get(key).push(t);
  });

  // For each group, pick the best (seeders > size)
  const bestSingles = [];
  for (let [key, group] of epGroups) {
    ...
    bestSingles.push(sorted[0]);
  }

  // For batches, group by normalized (without episode) and pick best
  ...
  return combined.slice(0, 30).map(t => {
    // Build description (episode info)
...
// ... (rest of endpoints unchanged: /api/trending, /api/search, /api/details, etc.)
// They remain exactly as in the previous version.
