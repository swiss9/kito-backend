...
async function getTorrents(title, category, isMovie = false, source = 'auto', episode = null) {
  ...
  const fetchFn = async () => {
    ...
    // Determine fallback chain based on category type
    if (category === 'Manga') {
      // Manga doesn't have torrents
      const result = { results: [], source: 'none', message: 'Manga are not available for download. Please check official sources.' };
      setCache(cacheKey, result);
      return result;
    }

    const animeLike = ['Anime', 'Tokusatsu'];
    const movieLike = ['Hollywood', 'Bollywood', 'Animation', 'Asian'];

    if (animeLike.includes(category) || !isMovie) {
      // Series (Anime, Tokusatsu) or unknown
      fallbackChain = ['torrentclaw', 'nyaa', 'eztv', 'torrentsearchapi'];
    } else if (movieLike.includes(category) || isMovie) {
...
app.get('/api/torrents', async (req, res) => {
  try {
    ...
    const movieCategories = ['Hollywood', 'Bollywood', 'Animation', 'Asian'];
    const seriesCategories = ['Anime', 'Tokusatsu'];
    const isMovie = movieCategories.includes(category);
    const isSeries = seriesCategories.includes(category);

    let searchTitle = title;
    if (category === 'Manga') {
      // Manga has no torrents
      return res.json({
        title,
        category,
        source: 'none',
    ...
    // For movies: do not append "batch"; for series: append "batch" if no episode
    if (episode) {
      searchTitle = `${title} ${episode}`;
    } else if (isSeries) {
      searchTitle = `${title} batch`;
    }
    // else: for movies, searchTitle stays as title (no batch)

    const result = await getTorrents(searchTitle, category, isMovie, source, episode);
