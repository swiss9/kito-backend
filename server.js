...
try {
  ...
  console.warn('cheerio not installed, Nyaa scraper will be disabled');
}
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    if (!q) return res.json({ query: q, category: categoryId, items: [], total: 0 });
    ...
    for (let catId of categories) {
      ...
      if (config.metadataProvider === 'anilist') {
        ...
        `;
        const type = 'ANIME';
        const variables = { search: q, type, perPage: limit, page: page };
        const data = await fetchAniList(query, variables);
        const items = (data.Page.media || []).map(item => {
          const media = normalizeAniListMedia(item, catId);
          return mediaToCardArray(media);
