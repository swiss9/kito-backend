...
// ---------- Helper functions (same as before, but no file I/O) ----------
    ...
    for (const catId of categories) {
      ...
      if (config.metadataProvider === 'anilist') {
        try {
          ...
          console.error(`AniList error for ${catId}:`, err.message);
        }
      }
      ...
      if (config.metadataProvider === 'tmdb' && TMDB_API_KEY) {
        try {
          ...
          console.error(`TMDB error for ${catId}:`, err.message);
        }
      }
    }
...
app.get('/api/test', async (req, res) => {
  try {
    ...
    `;
    const result = await fetchAniList(testQuery, {});
