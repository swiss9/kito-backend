...
function mediaToCardArray(media) {
  ...
  return [media.title, sub, getGradientClass(media.title), false, media.poster, media.id];
}
...
function isBatchRelease(name) {
  ...
  return /batch|complete|全集|season pack|all episodes|full season/i.test(lower);
}
...
function scoreRelease(release, media) {
  ...
  if (titleMatch || altMatch) {
    ...
    // If no title match, penalize heavily
    score -= 200;
  }
...
function deduplicateAndRank(torrents, media) {
  ...
  const batches = enriched.filter(t => t.isBatch || t.range);
  const singles = enriched.filter(t => !t.isBatch && !t.range);
  ...
  if (batches.length > 0) {
    ...
    for (let [key, group] of batchGroups) {
      ...
      const best = scored[0];
      // Only include if score is positive (title match)
      if (best.score > 0) {
        results.push({
          name: best.name,
          magnet: best.magnet,
          ...
          description: best.range ? `Episodes ${best.range.start}-${best.range.end} · Size: ${best.size}` : `Batch · Size: ${best.size}`,
          score: best.score
        });
      }
    }
  ...
  // Process singles only if no batches found
  if (singles.length > 0 && results.length === 0) {
    ...
    for (let [key, group] of epGroups) {
      ...
      const best = scored[0];
      if (best.score > 0) {
        const epDisplay = best.episode !== null ? `Episode ${best.episode}` : '';
        results.push({
          name: best.name,
          magnet: best.magnet,
          size: best.size,
          ...
          type: 'single',
          quality: best.quality,
          description: `${epDisplay} · Size: ${best.size}`,
          score: best.score
        });
      }
    }
    ...
    results.sort((a, b) => {
      const aEp = parseInt(a.description.match(/Episode (\d+)/)?.[1] || 0);
      const bEp = parseInt(b.description.match(/Episode (\d+)/)?.[1] || 0);
      if (aEp && bEp) return aEp - bEp;
      return b.score - a.score;
    });
  }

  // Final sort: batches first, then by score
  results.sort((a, b) => {
  ...
  try {
    ...
    for (let catId of categories) {
      ...
      if (config.metadataProvider === 'anilist') {
        ...
        const mediaType = config.mediaType === 'movie' ? 'movie' : 'tv';
        const url = `https://api.themoviedb.org/3/search/${mediaType}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(q)}&language=en-US&page=${page}`;
        const res = await fetch(url, { signal: AbortSignal.timeout(8000) });
        if (res.ok) {
          const data = await res.json();
