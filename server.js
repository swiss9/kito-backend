...
function isBatchRelease(name) {
  ...
  if (/batch|complete|全集|season pack|all episodes|full season|season \d+ (complete|full)|s\d+ (complete|full)|complete series|complete collection|complete set|complete box|box set/i.test(lower)) return true;
  if (extractEpisodeRange(name)) return true;
  const rangeMatch = name.match(/(\d{3})\s*[-–]\s*(\d{3})/);
  if (rangeMatch) {
    const start = parseInt(rangeMatch[1]);
    const end = parseInt(rangeMatch[2]);
    if (start > 0 && end > 0 && end - start >= 10) return true;
  }
  return false;
}
...
const SPINOFF_BLACKLIST = ['boruto', 'rock lee', 'spectacle', 'live'];
...
function deduplicateAndRank(torrents, media) {
  ...
  const titleVariants = allTitles.map(t => t.toLowerCase());

  const filtered = enriched.filter(t => {
    const name = t.name.toLowerCase();

    const hasTitleVariant = titleVariants.some(v => name.includes(v));
    if (!hasTitleVariant) return false;

    const isSpinoff = SPINOFF_BLACKLIST.some(keyword => name.includes(keyword));
    if (isSpinoff) {
      const hasSpinoffInMedia = titleVariants.some(v => v.includes('boruto') || v.includes('rock lee') || v.includes('spectacle'));
      if (!hasSpinoffInMedia) return false;
    }

    if (media.episodes && media.episodes > 0) {
      if (t.episode !== null && t.episode > media.episodes) return false;
      if (t.range && t.range.start > media.episodes) return false;
    }

    // For batches: very permissive
    if (t.isBatch) {
      // If the torrent name contains any title variant, accept it.
      const hasTitle = titleVariants.some(v => name.includes(v));
      if (hasTitle) return true;
      // Otherwise, use very low threshold
      const sim = jaccardSimilarity(t.name, allTitles.join(' '));
      return sim >= 0.02;
    }

    const threshold = media.category === 'anime' ? 0.08 : 0.15;
    const sim = jaccardSimilarity(t.name, allTitles.join(' '));
...
async function searchAnimeReleases(media) {
  ...
  if (media.titles?.romaji) titles.add(media.titles.romaji);
  if (media.titles?.english) titles.add(media.titles.english);
  if (media.titles?.native) titles.add(media.titles.native);

  const normalizedTitles = new Set();
  for (const t of titles) {
    const clean = t.replace(/[:]/g, '').replace(/\s+/g, ' ').trim();
    normalizedTitles.add(clean);
    normalizedTitles.add(`${clean} complete`);
    normalizedTitles.add(`${clean} complete series`);
    normalizedTitles.add(`${clean} complete collection`);
    // Add with year in parentheses
    if (media.year) {
      normalizedTitles.add(`${clean} (${media.year})`);
      normalizedTitles.add(`${clean} (${media.year}) complete series`);
      normalizedTitles.add(`${clean} (${media.year}) complete`);
    }
  }
  ...
  normalizedTitles.forEach(title => {
    queries.push(title);
    if (media.year) {
      queries.push(`${title} ${media.year}`);
      queries.push(`${title} ${media.year} complete series`);
      queries.push(`${title} ${media.year} complete collection`);
    }
    queries.push(`${title} batch`);
    queries.push(`${title} complete`);
    queries.push(`${title} complete series`);
    queries.push(`${title} complete collection`);
    if (media.episodes) {
      queries.push(`${title} 1-${media.episodes}`);
      queries.push(`${title} ${media.episodes}`);
    }
    if (media.year) {
      queries.push(`${title} ${media.year} batch`);
      queries.push(`${title} ${media.year} complete`);
    }
  });
  ...
  if (media.year) {
    titles.forEach(t => {
      ...
      queries.push(`${clean} (${media.year})`);
      queries.push(`${clean} (${media.year}) complete series`);
    });
  }
  ...
  if (rawResults.length === 0) {
    ...
    titles.forEach(title => {
      ...
      fallbackQueries.push(`${clean} complete series`);
      fallbackQueries.push(`${clean} complete collection`);
      fallbackQueries.push(`${clean} 1-${media.episodes || 500}`);
      if (media.year) {
        fallbackQueries.push(`${clean} ${media.year} complete series`);
        fallbackQueries.push(`${clean} (${media.year}) complete`);
      }
      // Known groups
      fallbackQueries.push(`AnimeRG ${clean}`);
    });
    for (const q of fallbackQueries) {
      try {
        const results = await searchNyaa(q);
        if (results.length > 0) {
          const ranked = deduplicateAndRank(results, media);
          if (ranked.length > 0) return ranked.slice(0, 100);
        }
      } catch {}
    }
    return [];
...
async function searchMovieReleases(media) {
  ...
  titles.forEach(title => {
    queries.push(title);
    if (media.year) queries.push(`${title} ${media.year}`);
  });
