...
async function fetchEZTV(title) {
  // EZTV API is simple: search via GET
...
async function getTorrents(title, category, source = 'auto') {
  ...
  const fetchFn = async () => {
    ...
    if (category === 'Anime' || category === 'Manga') {
      ...
      // Movies/TV: TorrentClaw → TorrentAPI → EZTV (if TV)
      fallbackChain = ['torrentclaw', 'torrentapi'];
      // If category is TV, also try EZTV
      if (category === 'Hollywood' || category === 'Animation' || category === 'Asian' || category === 'Bollywood') {
