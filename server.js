...
function isBatchRelease(name) {
  ...
  if (/batch|complete|全集|season pack|all episodes|full season|season \d+ (complete|full)|s\d+ (complete|full)/i.test(lower)) return true;
...
// Known spin‑off keywords to block when they aren't in the media's own titles
...
function deduplicateAndRank(torrents, media) {
  ...
  // Build a set of lowercase title variants for substring check
  const titleVariants = allTitles.map(t => t.toLowerCase());

  const filtered = enriched.filter(t => {
    const name = t.name.toLowerCase();

    // 1. Must contain at least one of the media's title variants as a substring
    const hasTitleVariant = titleVariants.some(v => name.includes(v));
    if (!hasTitleVariant) return false;

    // 2. Block known spin‑off keywords if they aren't part of the media's title
    ...
    // 3. Episode number/range must be within the series' episode count
    ...
    // 4. Jaccard similarity (threshold raised to 0.15)
...
async function searchAnimeReleases(media) {
  ...
  if (rawResults.length === 0) {
    if (media.titles?.native && !titles.has(media.titles.native)) {
      try {
        const fallbackResults = await searchNyaa(media.titles.native);
        if (fallbackResults.length > 0) {
          const ranked = deduplicateAndRank(fallbackResults, media);
