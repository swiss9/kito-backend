...
function validateRelease(parsed, media) {
  ...
  if (media.title === 'Kamen Rider' && year === 1971) {
    if (releaseNorm.includes('1971') && releaseNorm.includes('softshout') && releaseNorm.includes('4kbd')) {
      console.log(`[validateRelease] Hardcoded 1971 SoftSHOUT accepted: ${parsed.originalName}`);
