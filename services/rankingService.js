...
function titleMatchesStrict(releaseTitle, mediaTitles, media) {
  ...
  console.log(`[titleMatchesStrict] Release: "${releaseTitle}"`);

  // Special case: Kamen Rider 1971 (using year from media object)
  const isKamenRider1971 = media.title.includes('1971') || media.year === 1971;
  if (isKamenRider1971) {
    ...
    // You can modify this to accept any release with "1971" and no other series
  ...
  if (mediaTitleNorm.includes('kamen rider') || mediaTitleNorm.includes('masked rider')) {
    ...
    // Fallback: if release contains "kamen rider" and no other series, accept
  ...
  // Special cases for other problematic titles
  ...
  // General matching for other anime/movies
  for (const mt of mediaTitles) {
    if (!mt) continue;
    const normMt = normalizeTitleForMatching(mt);
    if (releaseNorm.includes(normMt)) {
