...
function getAliases(media) {
  const baseAliases = [media.title, ...(media.aliases || [])].map(t => normalizeTitleForMatching(t));
  const result = [];
  const seen = new Set();
  ...
  if (ALIAS_MAP && typeof ALIAS_MAP === 'object') {
    const titleNorm = normalizeTitleForMatching(media.title);
    const allSearchTexts = [titleNorm, ...baseAliases];
    for (const [key, values] of Object.entries(ALIAS_MAP)) {
      let matchFound = false;
      for (const text of allSearchTexts) {
        if (wordMatch(text, key)) {
          matchFound = true;
          break;
        }
        for (const val of values) {
          if (wordMatch(text, val)) {
            matchFound = true;
            break;
          }
        }
      ...
      if (matchFound) {
        const normalizedKey = normalizeTitleForMatching(key);
        if (!seen.has(normalizedKey)) {
          seen.add(normalizedKey);
          result.push(normalizedKey);
        }
        for (const val of values) {
          const normalizedVal = normalizeTitleForMatching(val);
          if (!seen.has(normalizedVal)) {
            seen.add(normalizedVal);
            result.push(normalizedVal);
          }
        }
      }
    }
...
function hasOtherKamenRiderWork(releaseNorm, workName) {
  ...
  for (const other of OTHER_SERIES) {
    ...
    if (wordMatch(releaseNorm, other)) return true;
  }
  return false;
}
...
function isBatchRelease(name, media) {
  ...
  if (lower.includes('complete season')) return true;
  if (lower.includes('season collection')) return true;
...
function validateRelease(parsed, media) {
  ...
  const originalNorm = normalizeTitle(parsed.originalName).normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  const originalNormLower = normalizeTitleForMatching(originalNorm);

  if (media.title === 'Kamen Rider' && year === 1971) {
    if (wordMatch(releaseNorm, '1971') && wordMatch(releaseNorm, 'softshout') && wordMatch(releaseNorm, '4kbd')) {
      logger.debug(`[validateRelease] Hardcoded 1971 SoftSHOUT accepted: ${parsed.originalName}`);
      return { valid: true, confidence: 0.95, reasons: ['hardcoded_1971_softshout'], episodeStart: null, episodeEnd: null };
    }
  }
  ...
  for (const mt of mediaTitles) {
    if (wordMatch(releaseTitle, mt) || wordMatch(originalNorm, mt)) {
  ...
  if (!exactTitleMatch) {
    ...
    for (const alias of aliases) {
      if (wordMatch(releaseNorm, alias) || wordMatch(originalNormLower, alias)) {
        aliasMatch = true;
        confidence += 0.25;
    ...
    if (!aliasMatch && workName && (wordMatch(releaseNorm, workName) || wordMatch(originalNormLower, workName))) {
      confidence += 0.2;
  ...
  if (franchise) {
    if (wordMatch(releaseNorm, franchise) || wordMatch(originalNormLower, franchise)) {
      confidence += 0.05;
  ...
  const isBatchCandidate = /batch|complete|collection|season\s*[\d-]+/i.test(parsed.originalName);
  ...
  if (year) {
    ...
    if (yearRange) {
      ...
      const endYear = parseInt(yearRange[2]);
      if (startYear <= year && endYear >= year) rangeMatches = true;
    }
    if (yearFound || rangeMatches) {
      confidence += 0.05;
      reasons.push('year_match');
    } else if (isBatchCandidate) {
      reasons.push('year_missing_in_batch');
    } else {
  ...
  for (const rel of related) {
    if (wordMatch(releaseNorm, rel) || wordMatch(originalNormLower, rel)) {
      confidence += 0.05;
  ...
  if (franchise === 'kamen rider') {
    ...
    if (effectiveWorkName === '1971') {
      const has1971 = wordMatch(releaseNorm, '1971') || wordMatch(originalNormLower, '1971');
      const hasOther = hasOtherKamenRiderWork(releaseNorm, '1971') || hasOtherKamenRiderWork(originalNormLower, '1971');
      if (!has1971 || hasOther) {
        valid = false;
      ...
      const hasWork = wordMatch(releaseNorm, effectiveWorkName) || wordMatch(originalNormLower, effectiveWorkName);
      if (!hasWork) {
        const isExact = mediaTitles.some(mt => wordMatch(releaseTitle, mt) || wordMatch(originalNorm, mt));
        if (!isExact) {
          valid = false;
  ...
  for (const forb of forbiddenTitles) {
    if (wordMatch(releaseNorm, forb) || wordMatch(originalNormLower, forb)) {
      const mediaContainsForbidden = mediaTitles.some(mt => wordMatch(releaseNorm, mt) || wordMatch(originalNormLower, mt));
  ...
  const extraWords = releaseTokens.filter(t => !mediaTokens.includes(t) && t.length > 2);
  if (extraWords.length > 2) {
    reasons.push('too_many_extra_words');
    return { valid: false, confidence: 0, reasons };
  }

  const releaseSeason = parsed.season;
  const mediaSeason = getMediaSeason(media);
  if (releaseSeason !== null && releaseSeason !== mediaSeason) {
    const isBatch = /batch|complete|collection|season\s*[\d-]+/i.test(parsed.originalName);
    if (!isBatch) {
      valid = false;
...
function calculateScore(parsed, coverageType, coveragePercent, media, validation) {
  ...
  if (parsed.originalName && parsed.originalName.toLowerCase().includes('complete season')) score += 10;
  if (parsed.originalName && parsed.originalName.toLowerCase().includes('season collection')) score += 10;
...
function processRelease(rawRelease, media) {
  ...
  if (!validation.valid) {
    logger.debug(`[reject] ${rawRelease.name} -> ${validation.reasons.join(', ')}`);
    return null;
  }
